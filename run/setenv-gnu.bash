...
#
# Todas as variáveis usam o idioma "${VAR:-default}": defina-as no ambiente
# ANTES do source para sobrescrever os caminhos padrão.
#
...
#   make / make clean / make distclean / make rebuild
...
# ── Verificadores (removidos ao final) ────────────────────────────────────────
_OK=0 ; _MISS=0
_chk_file() {
...
# ── Raiz do acoplador, derivada da localização do script (run/ → ..) ──────────
_COUPLER_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

# ── [1] ESMF 8.9.1 — ponto único: ESMFMKFILE (incluído pelo Makefile) ─────────
# ESMF_LIBDIR deriva sempre do diretório do esmf.mk (robusto a override).
...
# ── [2] MONAN-A 2.0 — artefatos consolidados em mod/monan2 e lib/monan2 ───────
...
# ── [3] RRTMG big-endian (sem isto, leitura de RRTMG_SW_DATA.DBL aborta) ──────
export GFORTRAN_CONVERT_UNIT='big_endian:101'

# ── [4] MOM6+SIS2 — raiz padrão é o próprio acoplador (lib/{mom6,fms,nuopc}) ──
...
# MOAB interno ao libesmf.so: padrão 'no'. 'yes' exige MOAB_DIR no ambiente.
export USE_EXTERNAL_MOAB="${USE_EXTERNAL_MOAB:-no}"

# pNetCDF: prefixo do módulo Cray, com fallback sobrescrevível.
...
# ── [5] MOM6_HDR_INC — cabeçalhos (.h) para recompilar os caps upstream ───────
# MOM_memory.h e version_variable.h vivem na árvore de fontes, não em include/.
...
if [[ -z "${MOM6_HDR_INC:-}" ]]; then
  ...
  [[ -z "${_mommem}" ]] && \
    _mommem=$(find "${MOM6_SRC}" "${MOM6_ROOT}" -name MOM_memory.h 2>/dev/null | head -1 || true)
  _vervar=$(find "${MOM6_SRC}" "${MOM6_ROOT}" -name version_variable.h 2>/dev/null | head -1 || true)
  MOM6_HDR_INC=""
...
# ── [6] LD_LIBRARY_PATH — ordem de prioridade: NUOPC→MOM6→FMS→MOAB→MONAN→ESMF ─
# Idioma "${VAR:+:${VAR}}" evita trailing colon (que incluiria '.' na busca).
export LD_LIBRARY_PATH="${MONAN2_LIBDIR}:${ESMF_LIBDIR}${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
...
if [[ -z "${MOM6_HDR_INC:-}" ]]; then
  printf "  %-26s: ${_CV_AM}%s${_CV_RS}\n" "MOM6_HDR_INC" "NÃO ENCONTRADO — caps upstream não compilarão"
else
