...
- **Continentes mascarados em `mom6_import_*.nc` e `monan2_import_*.nc` (MASCARA-CONT-01/02).** Nenhum dos dois diagnósticos de importação sabia, célula a célula, o que era continente. `mom6_import_*.nc` não mascarava terra nenhuma; `monan2_import_*.nc` só descartava valores fora de faixa física, o que não pega terra porque a célula de terra recebe um valor plausível (`So_t≈271,35 K`). O sintoma já estava documentado nos próprios scripts de pós-processamento — `BUG-PY-18` em `postproc_mom6_import.py` e `BUG-14` em `postproc_monan2_import.py` — como um heurístico por igualdade aproximada com esse valor, que tinha o efeito colateral de apagar SST real sob gelo marinho (que também fica perto do ponto de congelamento).

  A correção usa a máscara real de cada modelo. No lado MOM6, `So_omask = nint(mask2dT)` (já exportado ao mediador, FIX B-COASTMASK-02) é regredido uma única vez para a grade ATM 360×180 pelo route handle bilinear já existente `is%rh_ocn2atm`, produzindo uma fração de cobertura oceânica por célula em `is%ocn_mask_atm` (campo que já existia no tipo `MED_InternalState`, mas nunca era preenchido). `med_write_import_fields` grava `_FillValue` onde essa fração fica abaixo de 0,5. No lado MPAS, `xland` (subpool `sfc_input`, a mesma máscara já usada em `mpas_atm_run` para decidir onde aplicar SST importada) passa a ser exposto em `mpas_atm_public_type` e encaminhado por `mpas_import` até `write_mpas_import_diag`; `voronoi_to_grid` passa a calcular, por célula de saída, a fração de pontos-fonte Voronoi que são água, em vez da fração de dado fisicamente válido, e um ponto de terra deixa de entrar na média mesmo quando o valor está dentro da faixa física.

  Os dois arquivos passam a gravar também uma variável `ocn_frac`, com a fração usada para decidir o corte — célula a célula, para transparência, e para expor a faixa costeira (valores entre 0 e 1) que se perdia por completo no esquema anterior. Enquanto a máscara real ainda não está disponível (bootstrap no MOM6; `xland` ausente no MPAS), o comportamento recua automaticamente ao anterior — sem máscara — em vez de apagar dado por falta de uma informação ainda não pronta.

  `postproc_mom6_import.py` (v9.2) e `postproc_monan2_import.py` (v3.2) desligam por padrão o heurístico por temperatura, agora redundante nos arquivos novos; `--legacy-land-marker` religa o comportamento antigo para arquivos gerados antes desta revisão (sem a variável `ocn_frac`). `analisa_sst_ifrac.py` não precisou de nenhuma mudança, porque já unia a máscara automática do netCDF4 (`_FillValue`) com o heurístico. Desenho completo, com diagrama, em `docs/mascara-continentes.md`.
